from extra import resource_path, timeCal, data_size_cal, genSessionId, DataBabse, isOnline
...
class YouTube_Download():
    ...
    def __init__(self, parent_window: Tk, root):
        self.width = 1200
        self.height = 800
    ...
    def DownloadPlaylist(self, video_name):
        ...
        try:
            ...
            if not isOnline():
                ...
                messagebox.showerror("YouTube Error", f"     {e}")
            return 0
    ...
    def search_entry(self,event=None):
        ...
        if "https://www.youtube.com/playlist?" in  video_name:
            t=Thread(target=self.DownloadPlaylist,args=(video_name,) , daemon=True)
            t.start()
            #url|||resulution|||[1,2,3]
        elif "--v--" in video_name:
            video_name=video_name.replace("--v--","")
            if "https://www.youtube.com/watch?v=" in video_name:
                video_name=video_name.split("watch?v=")[1]
            if len(video_name) ==11:
                self.video_id=video_name
                th=Thread(target=self.call_download_menu,args=(event,self.video_id,),daemon = True)
                th.start()    
         
            else:
                print("Vidoe====>",video_name)
    ...
    def call_download_menu(self,event=None, widget=None, i_d=None, tok=False): 
        ...
        self.show_load_pop()       
        download_menu = Menu(self.canvas,fg='black',bg='grey60',tearoff = 0, font=('',26))
        try:
            self.video=YouTube('https://www.youtube.com/watch?v='+i_d)#, use_oauth=True, allow_oauth_cache=True)
            print("======>https://www.youtube.com/watch?v="+i_d)

            
            download_menu.add_command(label = "music mp3",
                                    command =lambda : self.lownload_selected_video('mp3'))
            download_menu.add_separator()

            all_stream=self.video.streams.filter(progressive=True , file_extension='mp4')

            for stream in all_stream:
                download_menu.add_command(label =str(stream.resolution)+"/mp4 ✓ Size= "+data_size_cal(stream.filesize)
                    ,command =self.functools_partial(self.lownload_selected_video, stream.itag) )

            download_menu.add_separator()

            all_stream=self.video.streams.filter(adaptive=True,only_video=True,file_extension='mp4')
            for stream in all_stream:
            ...
            download_menu.add_separator()
            all_stream=self.video.streams.filter(adaptive=True,only_video=True,file_extension='webm')

            for stream in all_stream:
                    download_menu.add_command(label =str(stream.resolution)+"/mp4  Size= "+data_size_cal(stream.filesize_approx),
                    command =self.functools_partial(self.lownload_selected_video,stream.itag) )
            ...
            if not isOnline():
                ...
                if "streamingData":
                    ...
                    print(f"Error==> {error}")
                    messagebox.showerror("YouTube Error", f"     {error}")
    ...
    def lownload_selected_video(self,video_id=None,video_stream=None,widget=None):
        if video_stream==None:
            video_stream=self.video
            
        th=Thread(target=self.try_download_selected_video, args=(video_id,video_stream,widget),daemon = True)
        th.start()
        

    def try_download_selected_video(self,video_id,video_stream,widget, ddir=None):
        # try:                
        self.threading_lownload_selected_video(video_id,video_stream,widget=widget, ddir=ddir)        
        # except Exception as e:
        #     print('Exception==: ',e)
    ...
    def threading_lownload_selected_video(self,video_id=None,video_stream=None,widget=None, ddir=None):
        self.show_pop_win()
        ...
        def mycb( stream, chunk, bytes_remaining):

            nonlocal stream_status
            if stream_status==False:
                print('+++++++++++++++++++++++++++++++++++')
                raise UserWarning ('Paused_Song')
            
            nonlocal prog
            nonlocal prevTime , tSize , tSizeCal

            tt=round(time())
            recv=tSize - bytes_remaining

            try:
                speed=(len(chunk))//(tt-prevTime)
            ...
            t_left = f"{timeCal(eta)} left"
    
            text_="(%s of  %s ,  %s)"%(data_size_cal(recv) ,tSizeCal ,
                    data_size_cal(speed))
            prog.update(int(recv), t_left, text_)


        
        if video_id =='mp3':
            best=video_stream.streams.filter(only_audio=True , file_extension="webm")
            best=best[len(best)-1]
        else:
            best = video_stream.streams.get_by_itag(int(video_id))
        
        tSize=best.filesize
        tSizeCal = data_size_cal(tSize)
            

        
        name,ext =os.path.splitext(best.default_filename)
        name2=""
        l=0
        for w in name:
            if w ==" ":
        ...
        directory = f"{str(Path.home())}\\Videos/"

        if ddir:
            directory = ddir
        else:
            if video_id =='mp3':
                directory = f"{str(Path.home())}\\Music/"


        filename = f"{name} TubeDown{ext}"
        print("filename==w>",filename)

        file_path = f"{directory}{filename}"

        if widget == None:
            prog = self.popUpWin.add_prog(file_path)
            prog.update(timest="Waiting...")
        else:
            prog = widget


        #Adding_Video_Name_And_Size_To_Status_Bar    
        prog.config(max_size=tSize, pause_cmd=pause_command)
        if video_id =='mp3':
        ...
        if file_exists_satatus==False:
            ...
            video_stream.register_on_progress_callback(mycb) 
            is_progressive = best.is_progressive

            try:
                stream=best.download(filename=file_path)
            ...
            prog.update(value=int(tSize))
            
            if video_id =='mp3':
                # time_left.config(text='Converting...',fg='green')
                prog.update(timest='Converting...')

                file_path=self.convert_mp4_to_mp3(file_path) 
                ...
                try:
                    ...
                    audio.tags.add(TIT2(encoding=3, text=str(video_stream.title)))
                    audio.tags.add(TLEN(encoding=3, text=str((video_stream.length)*1000)))
                    audio.tags.add(TCON(encoding=3, text=str("Music")))
                    audio.tags.add(TPE1(encoding=3, text=str(video_stream.author)))
                    audio.tags.add(TALB(encoding=3, text=str("M_A_Music_Player__ID: "+video_stream.video_id)))#Album
                    audio.tags.add(TPUB(encoding=3, text=str(video_stream.author)))
                    audio.tags.add(WPUB(encoding=3, text="Aditya_Mukhiya_M_A_Music_Player"))
                    audio.tags.add(TDRL(encoding=3, text=str(video_stream.publish_date).split(" ")[0]))
                    ...
                    try:
                        video_thumbnails = self.thumbnails[video_stream.video_id]   
                    except:
                        video_thumbnails=video_stream.thumbnail_url 
                        print("Thumbnail====> 22==>",video_thumbnails)

 
                    try:              
                        response = requests_get(video_thumbnails)
...
##        #deleting_widgets
        ...
        if file_exists_satatus==True:
            ...
            text='Download complete ('+tSizeCal+')'
            prog.update(timest=text)

            self.db.load()
            self.db["downloads"].append(file_path)
            self.db.commit()
...
youtube=YouTube_Download(root, root)
